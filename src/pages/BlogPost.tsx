    ...
    <div className="bg-background min-h-screen pb-20 md:pb-0">
      ...
      <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 container mx-auto px-4 pb-10">
          ...
          >
            <Link to="/blog" className="inline-flex items-center gap-2 text-sm text-white/50 hover:text-primary transition-colors mb-6 group">
      ...
      <div className="container mx-auto px-4 max-w-3xl py-16">
        ...
        >
          ...
          >
            <h3 className="text-xl font-heading font-bold text-white mb-3">Ready to Experience the Difference?</h3>
            <p className="text-white/50 text-sm mb-6">Explore our full range of frequency wellness devices.</p>
        ...
        {(prevPost || nextPost) && (
          <div className="mt-16 pt-10 border-t border-white/10 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {prevPost ? (
              <Link to={`/blog/${prevPost.slug}`} className="group flex items-start gap-4 p-5 rounded-xl border border-white/5 hover:border-primary/30 hover:bg-white/2 transition-all">
                <ArrowLeft className="w-5 h-5 text-white/30 group-hover:text-primary shrink-0 mt-0.5 transition-colors" />
                <div>
                  <div className="text-xs text-white/30 mb-1 uppercase tracking-wider">Previous</div>
                  <div className="text-sm font-medium text-white group-hover:text-primary transition-colors line-clamp-2">{prevPost.title}</div>
                </div>
              </Link>
            ) : <div />}
            {nextPost ? (
              <Link to={`/blog/${nextPost.slug}`} className="group flex items-start gap-4 p-5 rounded-xl border border-white/5 hover:border-primary/30 hover:bg-white/2 transition-all sm:flex-row-reverse text-right">
                <ArrowRight className="w-5 h-5 text-white/30 group-hover:text-primary shrink-0 mt-0.5 transition-colors" />
                <div>
                  <div className="text-xs text-white/30 mb-1 uppercase tracking-wider">Next</div>
                  <div className="text-sm font-medium text-white group-hover:text-primary transition-colors line-clamp-2">{nextPost.title}</div>
                </div>
              </Link>
            ) : <div />}
          </div>
